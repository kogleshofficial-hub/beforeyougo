...
export default function MyChecklist() {
  ...
  useEffect(() => {
    ...
    setWorkspace(id);
    try {
      const raw = localStorage.getItem(`beforeyougo:my-checklist:${id}:v2`);
      if (raw) {
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem(`beforeyougo:my-checklist:${workspace}:v2`, JSON.stringify({ title, items })); } catch { /* still usable in memory */ }
  }, [title, items, hydrated, workspace]);
  ...
  return (
    <main>
      ...
      <section className="shell detail builder">
        ...
        <div className="panel">
          <span className="eyebrow">Personal workspace</span><h1>Build your own checklist.</h1><p className="lead">Not seeing your task? Start from scratch. Add exactly what you need to remember.</p>
          ...
          <p className="privacy"><span>🔒</span> This checklist is stored only in this private browser workspace. Other visitors cannot see it. If another person uses this same browser, have them choose “New private workspace” first.</p>
        </div>
      </section>
