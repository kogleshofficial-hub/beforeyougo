...
export default function GuidePage() {
  ...
  useEffect(() => {
    ...
    setWorkspaceId(id);
    const storageKey = `beforeyougo:checklist:${id}:${slug}:v3`;
    try {
      const saved = window.localStorage.getItem(storageKey);
      const parsed = saved ? JSON.parse(saved) : null;
      const builtIn = Array.isArray(parsed?.done) && parsed.done.length === guide.items.length ? parsed.done.map(Boolean) : guide.items.map(() => false);
      const custom = Array.isArray(parsed?.customItems) ? parsed.customItems.filter((item: CustomItem) => item && typeof item.text === "string" && typeof item.id === "string" && typeof item.done === "boolean") : [];
      setDone(builtIn);
  ...
  useEffect(() => {
    if (!guide || !hydrated || !workspaceId) return;
    try { window.localStorage.setItem(`beforeyougo:checklist:${workspaceId}:${slug}:v3`, JSON.stringify({ done, customItems })); } catch { /* checklist still works in memory */ }
  }, [done, customItems, guide, hydrated, workspaceId, slug]);
  ...
  const addCustom = (event: React.FormEvent) => { event.preventDefault(); const text = customText.trim(); if (!text) return; setCustomItems((current) => [...current, { id: crypto.randomUUID(), text, done: false }]); setCustomText(""); };
  ...
  const newWorkspace = () => { const id = crypto.randomUUID(); try { localStorage.setItem(WORKSPACE_KEY, id); } catch {} router.refresh(); window.location.reload(); };
      ...
      <section className="shell detail">
        ...
        <div className="detailgrid">
          <article className="panel">
            ...
            <h1>{guide.title}</h1><p className="lead">{guide.summary}</p>
            ...
            <form className="add-item" onSubmit={addCustom}><label htmlFor="custom-item"><strong>Add your own item</strong><span>Include anything personal you need to remember.</span></label><div className="add-row"><input id="custom-item" value={customText} onChange={(event) => setCustomText(event.target.value)} placeholder="e.g. Bring a pen, confirmation email…" maxLength={160} /><button type="submit" disabled={!customText.trim()}>Add item</button></div></form>
            <p className="privacy"><span>🔒</span> This checklist belongs to your private browser workspace. Other visitors cannot see your checkmarks or custom items. Use “New private workspace” when another person uses this same browser.</p>
            <h2>Recommended flow</h2><div className="flow">{guide.steps.map((step, index) => <div className="flowitem" key={step}><b>{String(index + 1).padStart(2, "0")}</b><span>{step}</span></div>)}</div>
            <h2>Official source</h2><div className="source"><div><strong>{guide.sourceName}</strong><div className="tiny">Primary reference · {guide.scope}</div></div><a href={guide.source} target="_blank" rel="noopener noreferrer">Open official source ↗</a></div>
          </article>
